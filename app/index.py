...
class GetUrlAll(Handler):
    ...
    def get(self):
        ...
        n = yield self.dbs.resultcollection.find({'project': 'gethost', 'result.valid': 0}).count()
        res = self.dbs.resultcollection.find({'project': 'gethost', 'result.valid': 0, 'result.url': {'$regex': self.keyword}}).limit(20).skip(20*(int(self.page)-1))
        self.results = []
        for item in (yield res.to_list(20)):

            self.results.append(item['result'])

        self.writejson({'data': self.results, 'total': n, 'currentpage': self.page})
